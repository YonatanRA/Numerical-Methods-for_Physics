function [T]=acero2  % método explícito ec difusion trabajo materia condensada
...
A(100,100)=1;
for j=2:99
    A(j,j-1)=w(j)*(3*k(j)-k(j+1))/2;
    A(j,j)=w(j)*(-2*k(j)+(1/w(j)));
    A(j,j+1)=w(j)*(k(j)+k(j+1))/2;
end
