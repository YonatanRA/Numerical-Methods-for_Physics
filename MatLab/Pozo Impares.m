%Pozo de potencial. Funciones impares.
clear all
clc
...
df1=zeros(1,nx);  % para inicializar las funciones
df1(1)=1;         % y las derivadas , y para poder
f2=zeros(1,nx);   % realizar posteriormente el shooting method.
df2=zeros(1,nx);  % Las condiciones iniciales corresponden
df2(1)=1;         % a las funciones impares.
f3=zeros(1,nx);
df3=zeros(1,nx);
df3(1)=1;
...
while abs(alfa-alfai)>tol; % Mientras la diferencia de 
    ...
    if f1(nx)*f2(nx)<0; % Primera parte shooting method.
        ...
        alfa=alfa+0.00001; % Segundo alfa=E/Vo=0.3836
    end
end


plot(u,f1);               % Representacion grafica.
title(['Funcion Impar']); % Se representa la mitad del pozo.
